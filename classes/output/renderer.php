...
class renderer extends \plugin_renderer_base {
    ...
    public function render_data_creation(data_creation $datacreation) {
        ...
        // zl_temp, add lom init

        if ($datacreation->contextlevel == CONTEXT_COURSE) {
            //$output .= \html_writer::start_tag('div', ['class' => 'adminuseractionhint']);
            $output .= get_string('or', 'lesson');
            $output .= \html_writer::div('');
            $output .= $this->single_button(new \moodle_url('/local/lom/index.php'), get_string('LOM-init', 'local_metadata'));
            $output .= \html_writer::end_tag('div');
            // zl_temm add delete
            $output .= get_string('or', 'lesson');
            $output .= \html_writer::div('');
            $output .= $this->single_button(new \moodle_url('/local/lom/reset.php'), get_string('LOM-delete', 'local_metadata'));
            $output .= \html_writer::end_tag('div');
        }
        return $output;
    }
